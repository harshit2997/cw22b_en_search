...
if __name__ == '__main__':
    ...
    single_query_search_times = []
    single_query_run_times = []
    ...
    with output_writer:
        ...
        for index, (topic_id, text) in enumerate(tqdm(query_iterator, total=len(topics.keys()))):
            ...
            single_query_run_times.append(single_query_end-single_query_start)
            single_query_search_times.append(single_query_search_end-single_query_start)

            assert len(single_query_run_times) == index+1
            assert len(single_query_search_times) == index+1

    print ()

    for q in [0.5, 0.75, 0.9, 0.95, 0.99]:
        lat = np.quantile(single_query_run_times, q)
        print (str(int(q*100))+"th percentile query run time = "+str(lat)+" seconds")

    print()

    for q in [0.5, 0.75, 0.9, 0.95, 0.99]:
        lat = np.quantile(single_query_search_times, q)
        print (str(int(q*100))+"th percentile query search time = "+str(lat)+" seconds")        