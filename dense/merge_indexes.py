...
parser.add_argument('--inputs', nargs='+', help='input indexes', required=True)
parser.add_argument('--output', type=str, help='output index path', required=True)
...
for i in range(len(args.inputs)):
    if len(args.inputs[i])<3:
        continue
    index = faiss.read_index(os.path.join(args.inputs[i], 'index'))
    docid_files.append(os.path.join(args.inputs[i], 'docid'))
