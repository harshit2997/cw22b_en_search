...
from pyserini.encode import JsonlRepresentationWriter, FaissRepresentationWriter
from custom_modules import JsonGzCollectionIterator
...
if __name__ == '__main__':
    ...
    input_parser.add_argument('--corpus', type=str, nargs='+',
    ...
    with embedding_writer:
        for path in args.input.corpus:
            if len(path)<5:
                continue
            collection_iterator = JsonGzCollectionIterator(path, args.input.fields, args.input.docid_field, delimiter)
            print ("Indexing "+ str(collection_iterator.size)+" documents")
        
            for batch_info in collection_iterator(args.encoder.batch_size, args.input.shard_id, args.input.shard_num):
                kwargs = {
                    'texts': batch_info['Clean-Text'],
            ...
            del collection_iterator
            gc.collect()


    #collection_iterator = JsonGzCollectionIterator(args.input.corpus, args.input.fields, args.input.docid_field, delimiter)
    #print ("Indexing "+ str(collection_iterator.size)+" documents")
    #with embedding_writer:
    #    for batch_info in collection_iterator(args.encoder.batch_size, args.input.shard_id, args.input.shard_num):
    #        kwargs = {
